...
##########################################################################################
# Parameters for MAP 20 NODES
##########################################################################################

# Fixed search scaffold for 20-node HPO
BASE_ENV_PARAMS = {
    'min_problem_size': 20,
    'max_problem_size': 20,
    'pomo_size': 20,
...
BASE_OPTIMIZER_PARAMS = {
    ...
    'scheduler': {
        # Scale LR drops to a 50-epoch HPO run so the optimizer decays in the
        # middle and late phases rather than following the old longer schedule.
        'milestones': [20, 35, 45],
        'gamma': 0.5,
    },
}
...
BASE_TRAINER_PARAMS = {
    ...
    # Shorter per-trial training keeps HPO affordable while still giving the
    # policy enough time to move beyond very early noisy updates.
    'epochs': 50,
    'train_episodes': 1500,
    'train_batch_size': 128,
    'critic_hidden_dim': 256,
    'ppo': {
        'epsilon': 0.29556004011058423,
        'ppo_epochs': 6,
        'gamma': 0.99,
        'lambda_future': 0.7691704511339827,
        'alpha_entropy': 0.0012065753342951555,
        'c_critic': 0.28220626191190656,
    },
    'validation': {
        'enable': True,
        'episodes': 400,
    ...
    'annealing': {
        'enable': True,
        # Start annealing in the final 20% of training so exploration remains
        # active for most of a 50-epoch run and tightens only near the end.
        'start_epoch': 40,
    ...
    'model_load': {
        'enable': False,
        'path': str(PROJECT_ROOT / 'results' / 'train_20nodes'),
        'epoch': 0,
    },
    'logging': {
...
RESULTS_BASE_DIR = PROJECT_ROOT / 'results' / 'optuna' / 'train_20nodes_auto'
DB_PATH = PROJECT_ROOT / 'results' / 'optuna' / 'fsmvrp_20node_auto_hpo.db'

OPTUNA_PARAMS = {
    'study_name': 'fsmvrp_20node_auto_run6_hpo',
    'storage': f"sqlite:///{DB_PATH.as_posix()}",
    # Increase search breadth to compensate for the shorter trial budget.
    'n_trials': 200,
    'timeout': None,
    'seed': 1234,
}
...
def _build_trial_configs(trial):
    ...
    #env_params['pomo_size'] = trial.suggest_categorical('pomo_size', [20, 30, 50, 80, 100])

    optimizer_params['optimizer']['lr'] = trial.suggest_float('lr', 1e-4, 1e-3, log=True)
    optimizer_params['optimizer']['weight_decay'] = trial.suggest_float('weight_decay', 1e-8, 1e-6, log=True)
    optimizer_params['scheduler']['gamma'] = trial.suggest_float('scheduler_gamma', 0.4, 0.9)

    #trainer_params['train_batch_size'] = trial.suggest_categorical('train_batch_size', [32, 64, 128])
    trainer_params['critic_hidden_dim'] = 256 #trial.suggest_categorical('critic_hidden_dim', [64, 128, 256]) #64
    trainer_params['ppo']['epsilon'] = trial.suggest_float('ppo_epsilon', 0.2, 0.3)
    trainer_params['ppo']['ppo_epochs'] = trial.suggest_int('ppo_epochs', 5, 6)
    trainer_params['ppo']['lambda_future'] = trial.suggest_float('lambda_future', 0.3, 0.65)
    trainer_params['ppo']['alpha_entropy'] = trial.suggest_float('alpha_entropy', 5e-4, 1.5e-2, log=True)
    trainer_params['ppo']['c_critic'] = trial.suggest_float('c_critic', 0.5, 1.0) #0.8
...
def _extract_val_metrics(trainer):
    ...
    candidate_keys = ['val_softmax_aug_score', 'val_score', 'val_argmax_aug_score']
...
def objective(trial):
    ...
    try:
        ...
        trial.set_user_attr('result_folder', metrics['result_folder'])
...
def _analyze_late_stage_improvement(completed_trials):
    # Ignore incomplete or None-valued trials and sort by trial number so the
    # comparison reflects study progress rather than ranking by objective value.
    valid_trials = [
    ...
    # Require at least a few completed trials so the 80/20 split is meaningful.
    ...
    # Split the completed trials into the first 80% and the last 20% to see
    # whether the best objective is still materially improving near the end.
    split_index = max(1, int(len(valid_trials) * 0.8))
    first_chunk = valid_trials[:split_index]
    last_chunk = valid_trials[split_index:]

    # Fall back gracefully when the split leaves no late-stage trials.
    if not last_chunk:
        return {
            'completed_trials': len(valid_trials),
            'enough_data': False,
    ...
    # Compute the best objective seen before the late segment and inside the
    # late segment to quantify whether the study is still improving.
    first_80pct_best = min(trial.value for trial in first_chunk)
    last_20pct_best = min(trial.value for trial in last_chunk)

    # Protect the ratio calculation from division-by-zero while keeping the
    # interpretation close to the original heuristic the user requested.
    denominator = max(abs(first_80pct_best), 1e-12)
    improvement = (first_80pct_best - last_20pct_best) / denominator

    # Turn the raw improvement into a simple recommendation you can act on
    # after each run without manually inspecting the study database.
    if improvement > 0.02:
...
def _write_study_reports(study):
    ...
    # Summarize whether the study is still improving near the end so we can
    # decide if the current trial budget is sufficient.
    ...
    # Append a compact convergence section so the recommendation appears in the
    # same plain-text report as the best trial summary.
    report_lines.append("")
...
def _create_study():
    return optuna.create_study(
        ...
        direction='minimize',
        storage=OPTUNA_PARAMS['storage'],
        load_if_exists=True,
        sampler=_load_auto_sampler(seed=OPTUNA_PARAMS['seed']),
        pruner=MedianPruner(n_startup_trials=15, n_warmup_steps=20, interval_steps=5, n_min_trials=5),
    )
...
def main():
    ...
    if completed_trials:
        ...
        # Log the convergence heuristic so you can quickly decide whether to
        # stop or extend the study without opening the generated report files.
        convergence_summary = _analyze_late_stage_improvement(study.trials)
...
def _set_debug_mode():
    ...
    #BASE_TRAINER_PARAMS['train_batch_size'] = 2
